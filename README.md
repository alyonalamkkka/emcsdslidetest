...
## Простая защита логином и паролем

В проекте есть `middleware.ts` с обычным HTTP Basic Auth. Это самый простой
вариант закрыть сервис "на пароль", пока нет полноценной авторизации.
...
браузер будет показывать стандартное окно логина/пароля перед входом в
приложение. Если переменные не заданы, сайт остаётся публичным.

Для Vercel добавь эти переменные в `Settings -> Environment Variables`, затем
сделай `Redeploy`.
